...
import { restockSchema, type RestockFormValues } from "@/validations/inventory"
...
export default function RestockModal({ isOpen, onClose, product }: RestockModalProps) {
  ...
  const form = useForm({
    ...
    onSubmit: ({ value }) => {
      adjustStock.mutate(
        ...
        {
          onSuccess: () => {
            showAlert({ variant: "success", message: "Stock restocked successfully" })
            onClose()
          },
          onError: () =>
            showAlert({ variant: "error", message: "Failed to restock. Please try again." }),
        },
      )
    },
  })
  ...
  useEffect(() => {
    ...
    form.setFieldValue("adjustmentType", "add")
  ...
  return (
    <Modal
      ...
      customHeader={
        <div className="border-borderSubtle flex items-center justify-between border-b px-6 py-5">
          <h2 className="font-jakarta text-brand text-base font-semibold">Restock</h2>
    ...
    >
      <div className="flex flex-col gap-4 px-6 py-5">
        ...
        <form.Field
          name="product"
          validators={{ onBlur: ({ value }) => validateField(restockSchema, "product", value) }}
        >
          {(field) => (
            <ProductSearchInput
              label="Product"
        ...
        <form.Field
          name="location"
          validators={{ onBlur: ({ value }) => validateField(restockSchema, "location", value) }}
        >
          {(field) => (
            <LocationSearchInput
              label="Location"
        ...
        <div className="grid grid-cols-2 gap-3">
          <form.Field
            name="adjustmentType"
            validators={{
              onBlur: ({ value }) => validateField(restockSchema, "adjustmentType", value),
            }}
          >
            {(field) => (
              <Dropdown
          ...
          <form.Field
            name="units"
            validators={{ onBlur: ({ value }) => validateField(restockSchema, "units", value) }}
          >
            {(field) => (
              <FormInput
                label="Units to add"
        ...
        <form.Field
          name="reason"
          validators={{ onBlur: ({ value }) => validateField(restockSchema, "reason", value) }}
        >
          {(field) => (
            <Dropdown
              label="Reason"
